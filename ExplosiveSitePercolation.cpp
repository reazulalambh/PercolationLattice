...
vector<double>wrapprob;
vector<int>countpc;
vector<double>occprob;
...
void static_cleaner()
{
    ...
    vector<double>().swap(wrapprob);
    vector<double>().swap(occprob);
    vector<int>().swap(countpc);
...
void wrap()
{
 ...
 ofstream fout (timestr);
    ...
    {
        wrapprob.push_back((long double)countpc[v]/(long double)E);
        h++;
    }
...
for(int y=0; y<occprob.size(); y++)
{
    ...
    fout1<<fixed<<setprecision(20)<<occprob[y]<<" "<<setprecision(20)<<wrapprob[y]<<endl;
    ...
    int main(int argc,char* argv[])
{
    ...
    M= (2*L*L);
    EMPTY = -M-1;
    wrapstr = "wrapsite_" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n" + to_string(filenum) + ".dat";
    timestr = "wraptimesite_" + to_string(mm) +"_L"+ to_string(L)+"_E"+ to_string(E)+".dat";
