...
void span()
{
   ...
   vector<long double>sizedistr(N,0.0);
 ...
 auto start = high_resolution_clock::now();
   ...
   ofstream file (distrstr);
    for(int v =0; v<N; v+=1)
    {
       file<<v+1<<"	"<<fixed<<setprecision(20)<<sizedistr[v]/(long double)(E*N)<<endl;
    }
...
int main(int argc,char* argv[])
{
    ...
    N= (L*L);
    M= (2*L*L);
    fractalstr = "fractal_" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n" + to_string(filenum) + ".dat";
    timestr = "wraptime_" + to_string(mm) +"_L"+ to_string(L)+"_E"+ to_string(E)+".dat";
	distrstr = "nspc_" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n" + to_string(filenum) + ".dat";
