...
vector<long double>secondlargest;
       ...
       if(x1!= y1)
       {
        ...
        int ll = findroot(x);
         if(- ptr[ll]>=big)
		 {
			 ...
			 if(x1!= firstroot && x1 != secondroot && y1 != firstroot && y1 != secondroot)
				...
				else if ((x1== firstroot && y1 == secondroot)|| (y1== firstroot && x1 == secondroot))
				{
					vector<int>ptrcopy;
					ptrcopy = ptr;
					
					nth_element(ptrcopy.begin(), ptrcopy.begin()+N-1-1,
					ptrcopy.end(), greater<int>());
					 secondbig= *(ptrcopy.begin()+N-1-1);

					auto it= find(ptr.begin(), ptr.end(), secondbig);
					
					secondroot = *it;
					vector<int>().swap(ptrcopy);
		 ...
		 if(secondbig<0){secondbig ==0;}
		 
    secondlargest [a+1] += (long double)secondbig/(long double)N;
...
void span()
{
 ...
 ofstream fout (timestr);
    ...
    for(int i=1; i<=M; i++)
  {
	  ...
	  secondlargest[i] = secondlargest[i]/(long double)E;
  }
    ofstream file1 (opstr);
    ofstream file2(entrstr);
	ofstream file3(secondlargestr);

    for(int y=0; y<occprob.size(); y++)
{

    file1<<fixed<<setprecision(20)<<occprob[y]<<" "<<setprecision(20)<<percstrength[y]<<endl;
	file3<<fixed<<setprecision(20)<<occprob[y]<<" "<<setprecision(20)<<secondlargest[y]<<endl;
 ...
 vector<long double>().swap(secondlargest);
...
int main(int argc,char* argv[])
{
	...
	secondlargestr = "secondlargest_"+ to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n"+ to_string(filenum) + ".dat";
    entrstr = "entropy_" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n"+ to_string(filenum) + ".dat";
    timestr = "opentime_" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ ".dat";
