...
double entropy_term(int i)
{
    long double meu=0;
           ...
           int s = -ptr[i];
    if(s>1){
           meu = ((long double)(s+0.5))* (logl((long double)s))  - ((long double)s) + 0.5 * logl(2*3.1416);}
...
void bondcreate()
{
 ...
 entropy[0] = (long double)logl(N) *(long double)(N+0.5) -(long double)N + 0.5 * logl(2*3.141592653589793238) ;
...
void shufflebonds()
{
   ...
   for (int i=0; i<=M; i++)
  {
   ...
   tempentropy[0] = (long double)logl(N) *(long double)(N+0.5) -(long double)N + 0.5 * logl(2*3.141592653589793238) ;
...
int main(int argc,char* argv[])
{
    ...
    entrstr = "entropy_b" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n"+ to_string(filenum) + ".dat";
    timestr = "opentime_b" + to_string(mm)+"_L"+ to_string(L) +"_E"+ to_string(E)+ "_n"+ to_string(filenum) + ".dat";
